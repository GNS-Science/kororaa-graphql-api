...
class ToshiHazardCurveQuery(graphene.ObjectType):
    ...
    args = graphene.Field(ToshiHazardCurveResult,
    ...
    def resolve_query(root, info, **kwargs):
        ...
        res = list(query.get_hazard_stats_curves(
            kwargs.get('toshi_hazard_id'),
            kwargs.get('imts'),
            kwargs.get('locs'),
            kwargs.get('aggs')))
        ...
        def build_response_from_query(query_response):
            ...
            for obj in query_response:
                yield ToshiHazardResult(toshi_hazard_id = obj.haz_sol_id,
                    imt=obj.imt, loc=obj.loc, agg=obj.agg, curve=get_curve(obj) )
