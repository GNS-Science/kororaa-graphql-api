...
class TestCustomPolygon(unittest.TestCase):
    def test_custom_class_is_hashable(self):
        ...
        pA = CustomPolygon(Polygon([(0, 0), (1, 1), (1, 0)]), 42)
        pB = CustomPolygon(Polygon([(0, 0), (1, 1), (1, 0)]), 84)
...
class TestGriddedHazard(unittest.TestCase):
    def test_get_clipped_grid(self):
        ...
        for pt in grid:
            tile = CustomPolygon(
                create_square_tile(region_grid.resolution, pt[1], pt[0]), random.randint(0, 4.7e6) / 1e6
            )
